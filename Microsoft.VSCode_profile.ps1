...
function Initialize-OhMyPosh {
    ...
    try {
        # Check if Oh My Posh is installed using cached command info
        ...
        if (-not $ohMyPoshCmd) {
            ...
            if ($AutoInstall) {
                ...
                # Check if winget is available using cached command info
                if (Get-Command winget -ErrorAction SilentlyContinue) {
                    ...
                    if ($Async) {
                        Write-Host "Installing Oh My Posh in background..." -ForegroundColor Yellow
                    } else {
                ...
                Write-Host "To install Oh My Posh automatically, run: Initialize-OhMyPosh -AutoInstall" -ForegroundColor Yellow
                return
            }
        }

        # Cache theme path to avoid repeated joins
        ...
        $Global:OhMyPoshCache = [ScriptBlock]::Create($initScript)
        
        # Execute from cache
        & $Global:OhMyPoshCache

        Register-ProfileMetric -Name "OhMyPosh-Init" -StartTime $startTime
        Write-Host "Oh My Posh initialized with theme: $ThemeName" -ForegroundColor Green

    } catch {
        Write-Error "Error initializing Oh My Posh: $_"
        Register-ProfileMetric -Name "OhMyPosh-Init" -StartTime $startTime -IsError -Details $_.Exception.Message
    }
}
...
function ai { Open-Urls -Urls $UrlCollections.AI.LLM -Message "Opened AI/LLM sites" }
function aidev { Open-Urls -Urls $UrlCollections.AI.AiPackages -Message "Opened AI development sites" }
function Open-AiSearch { Open-Urls -Urls $UrlCollections.AI.AiSearch -Message "Opened AI search engines" }
function Open-GoogleCore { Open-Urls -Urls $UrlCollections.Google.Core -Message "Opened Google core services" }
function Open-GoogleProductivity { Open-Urls -Urls $UrlCollections.Google.Productivity -Message "Opened Google productivity tools" }
function Open-GoogleMedia { Open-Urls -Urls $UrlCollections.Google.Media -Message "Opened Google media services" }
function Open-GoogleBusiness { Open-Urls -Urls $UrlCollections.Google.Business -Message "Opened Google business tools" }
function Open-DevDocs { Open-Urls -Urls $UrlCollections.Development.Documentation -Message "Opened developer documentation" }
function Open-DevGit { Open-Urls -Urls $UrlCollections.Development.GitSites -Message "Opened Git platforms" }
function Open-DevLearn { Open-Urls -Urls $UrlCollections.Development.LearnWebDev -Message "Opened web development learning resources" }
function Open-DevJavaScript { Open-Urls -Urls $UrlCollections.Development.Javascript -Message "Opened JavaScript resources" }
function Open-DevCss { Open-Urls -Urls $UrlCollections.Development.Css -Message "Opened CSS resources" }
function Open-DevPackages { Open-Urls -Urls $UrlCollections.Development.PackageManagers -Message "Opened package managers" }
function Open-DevCloud { Open-Urls -Urls $UrlCollections.Development.CloudSites -Message "Opened cloud platforms" }
function Open-FinanceStocks { Open-Urls -Urls $UrlCollections.Finance.StockSites -Message "Opened stock trading sites" }
function Open-FinanceTrading { Open-Urls -Urls $UrlCollections.Finance.Trading -Message "Opened trading platforms" }
function Open-FinanceForex { Open-Urls -Urls $UrlCollections.Finance.Forex -Message "Opened forex trading sites" }
function Open-FinanceCrypto { Open-Urls -Urls $UrlCollections.Finance.Crypto -Message "Opened cryptocurrency sites" }
function Open-FinanceBanking { Open-Urls -Urls $UrlCollections.Finance.Banking -Message "Opened banking sites" }
function Open-FinanceCards { Open-Urls -Urls $UrlCollections.Finance.CreditCards -Message "Opened credit card sites" }
function Open-NewsGeneral { Open-Urls -Urls $UrlCollections.News.NewsSites -Message "Opened news sites" }
function Open-NewsTech { Open-Urls -Urls $UrlCollections.News.TechNews -Message "Opened tech news sites" }
function Open-NewsMusic { Open-Urls -Urls $UrlCollections.News.Music -Message "Opened music services" }
function Open-SocialPro { Open-Urls -Urls $UrlCollections.Social.Professional -Message "Opened professional networks" }
function Open-SocialPersonal { Open-Urls -Urls $UrlCollections.Social.Personal -Message "Opened personal social media" }
function Open-SocialContent { Open-Urls -Urls $UrlCollections.Social.Content -Message "Opened content platforms" }
function Open-SocialCommunity { Open-Urls -Urls $UrlCollections.Social.Community -Message "Opened community sites" }
...
function Show-ProfileCommands {
    ...
    $commands = @{
        ...
        'ect' = @{
            ...
            'Get-Command | Select-Object Name' = 'List all available commands'
        }
    }
...
Initialize-OhMyPosh -AutoInstall -Async
...
# Add this to improve subsequent prompt performance

        ...
        if ($selectedOption) {
            ...
            elseif ($selectedOption.Path) {
                ...
                if (Test-Path $selectedOption.Path) {
                    Start-Process "explorer.exe" -ArgumentList $selectedOption.Path
            ...
            elseif ($selectedOption.URL) {
                # Open URL
                try {
                    Start-Process $selectedOption.URL
            ...
            Write-Host "Invalid choice. Please select a number between 0-15." -ForegroundColor Red
        }
    }
    catch {
